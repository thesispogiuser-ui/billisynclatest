...
const BUZZER_REF      = "billisync_buzzer";         // Arduino buzzer bridge
  ...
  function tryLogin(accounts) {
    ...
    if (isFirebase && db && USER.role==="member") {
      db.ref(ACCOUNTS_REF+"/"+USER.username).once("value").then(snap=>{
        ...
        if (fresh) { USER.bal = fresh.bal||0; persistSession(); }
        initDashboard();
      }).catch(()=>initDashboard());
    } else {
      initDashboard();
...
function initApp() {
  try {
    ...
    // Arduino Buzzer — listen for hardware feedback (Arduino → Website)
    db.ref(BUZZER_REF+"/lastBuzzed").on("value", snap=>{
      const ts = snap.val();
      if (!ts) return;
      const age = Date.now() - ts;
      if (age < 6000) {
        toast("🔔 Arduino Buzzer triggered!", "success");
      }
    });
...
function memberStopTable(id) {
  ...
  saveState(ns);
  triggerArduinoBuzzer("member_session_end");   // 🔔 Buzz Arduino — session ended
...
function shopForceStop(id) {
  ...
  saveState(ns);
  triggerArduinoBuzzer("shopkeeper_force_stop");  // 🔔 Buzz Arduino — shopkeeper ended session
  activeAlertIds.delete(id); alertedPlayedIds.delete(id);
  if (!activeAlertIds.size) stopAlert();
}
...
function confirmWalkinStop(id, cost) {
  ...
  saveState(ns);
  triggerArduinoBuzzer("walkin_session_end");   // 🔔 Buzz Arduino — walk-in session ended
  activeAlertIds.delete(id); alertedPlayedIds.delete(id);
  if (!activeAlertIds.size) stopAlert();
...
/* ============================================================
   ARDUINO BUZZER
   ============================================================ */
function triggerBuzzerFromWeb() {
  if (!isFirebase || !db) { toast("Firebase not connected. Cannot trigger buzzer.","warn"); return; }
  db.ref(BUZZER_REF).set({ command: "BUZZ", triggeredAt: Date.now(), lastBuzzed: null });
  toast("⏳ Buzzer command sent! Will ring in 5 seconds...","info");
}

// Internal helper — called automatically when a session ends or alert fires
function triggerArduinoBuzzer(reason) {
  if (!isFirebase || !db) return;
  db.ref(BUZZER_REF).set({ command: "BUZZ", triggeredAt: Date.now(), lastBuzzed: null, reason: reason||"session_end" });
  console.log("🔔 Arduino buzzer triggered:", reason);
}
...
function checkAlerts() {
  ...
  shouldAlert.forEach(id=>{
    if(!alertedPlayedIds.has(id)){
      alertedPlayedIds.add(id);
      playAlert();
      triggerArduinoBuzzer("time_alert");  // 🔔 Buzz Arduino — 3-min time warning
    }
  });
...
function renderMember(now, tables, logs, bal) {
  ...
  document.getElementById("m-nav").innerHTML = [
    ["main","📊 Dashboard"], ["tables","🎱 Tables"], ["reservations","🔖 My Reservations"], ["notifications","🔔 Alerts"], ["account","👤 Account"],
  ].map(([k,l])=>{
    const isCurrent = VIEW===k;
    const badge = k==="notifications" && getUnreadCount()>0 ? `<span class="notif-badge">${getUnreadCount()}</span>` : "";
    return `<button class="nav-btn ${isCurrent?"active-green":""}" onclick="setView('${k}')">${l}${badge}</button>`;
  }).join("");
  ...
  if (VIEW==="main") {
      ...
      myReservedTables.forEach(t=>{
        h+=`<div class="mini-card" style="border-color:var(--go)">
          <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
            <div>
              <div style="font-weight:700">🔒 ${t.name} <span style="font-size:11px;color:var(--go)">RESERVED FOR YOU</span></div>
              <div style="font-size:12px;color:var(--dm);margin-top:2px">Downpayment paid: <span style="color:var(--go);font-weight:600">${peso(t.downpayment||0)}</span></div>
              <div style="font-size:11px;color:var(--gr);margin-top:2px">✅ Table is locked — no one else can use it</div>
            </div>
            <div style="display:flex;gap:8px">
              <button class="btn btn-primary btn-sm" onclick="memberUseTable(${t.id})">▶ Start Now</button>
              <button class="btn btn-ghost btn-sm" onclick="cancelAvailableReservation(${t.id})">Cancel</button>
            </div>
          </div>
        </div>`;
      });
    ...
    h+=`<div class="section-header">
      <div class="section-title">🔔 Notifications ${unread.length?`<span class="notif-badge-lg">${unread.length} new</span>`:""}
      </div>
      ${notifications.length ? `<button class="btn btn-ghost btn-sm" onclick="markAllNotifsRead()">Mark all read</button>` : ""}
    </div>`;
    if (!notifications.length) {
      h+=`<div class="empty-state"><div class="icon">🔔</div><div class="title">No notifications</div><div class="sub">You'll be notified when your balance is updated.</div></div>`;
    } else {
      h+=`<div class="notif-list">`;
      notifications.forEach(n=>{
        const isBalance = n.type==="balance_added"||n.type==="balance_set";
        const icon = isBalance ? "💰" : "📢";
        h+=`<div class="notif-item ${n.read?"notif-read":"notif-unread"}">
          <div class="notif-icon">${icon}</div>
          <div class="notif-body">
            <div class="notif-msg">${n.message}</div>
            <div class="notif-time">${timeStr(n.time)} · ${dateStr(n.time)}</div>
          </div>
          <div class="notif-actions">
            ${!n.read ? `<button class="btn btn-ghost btn-sm" onclick="markNotifRead('${n.key}')">✓</button>` : ""}
            <button class="notif-dismiss" onclick="dismissNotif('${n.key}')">×</button>
          </div>
        </div>`;
      });
      h+=`</div>`;
    ...
    h+=`<div style="max-width:500px">
      <div class="profile-card">
        <div class="profile-header">
          ...
          <div><div class="profile-name">@${USER.username}</div><div class="profile-sub">Member</div></div>
        </div>
        <div class="profile-stats">
          <div><div class="label" style="font-size:10px;font-weight:600;color:var(--dm);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px">Balance</div><div style="font-size:22px;font-weight:700;color:${myBal>0?"var(--gr)":"var(--dn)"}">${peso(myBal)}</div></div>
          <div><div class="label" style="font-size:10px;font-weight:600;color:var(--dm);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px">Total Sessions</div><div style="font-size:22px;font-weight:700">${myLogs.filter(l=>l.type==="start").length}</div></div>
        </div>
      </div>
...
function renderShop(now, tables, logs, bal) {
  ...
  if (VIEW==="main") {
    ...
    h+=`<div class="ctrl-grid">
      ...
      <div class="ctrl-card">
        <div class="ctrl-card-title">🔔 Arduino Buzzer</div>
        <div class="ctrl-card-sub">Trigger the physical buzzer remotely from the website.</div>
        <button class="btn btn-primary btn-full" onclick="triggerBuzzerFromWeb()">📡 Trigger Buzzer</button>
      </div>
    </div>`;
  }
