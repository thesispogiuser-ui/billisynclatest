    ...
    h+=`<div style="max-width:500px">
      <div class="profile-card">
        <div class="profile-header">
          <div class="profile-avatar" style="font-size:32px;width:60px;height:60px;display:flex;align-items:center;justify-content:center;background:rgba(80,200,80,.1);border-radius:50%;border:2px solid var(--gr)">👤</div>
          <div>
      ...
      <div class="mini-card" style="margin-bottom:12px">
        <div style="font-size:13px;font-weight:700;color:var(--dm);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">✏️ Edit Display Name</div>
        <label class="form-label">Display Name</label>
        <input class="input" id="edit-name-inp" value="${USER.name||USER.username}" placeholder="Your display name" style="margin-bottom:10px">
        <div id="edit-name-err" class="login-error hide"></div>
        <button class="btn btn-primary btn-sm" onclick="saveDisplayName()">Save Name</button>
      </div>
...
function saveDisplayName() {
  ...
  setError("edit-name-err","");
  if (!newName) { setError("edit-name-err","Name cannot be empty."); return; }
  if (!isFirebase || !db) { USER.name=newName; persistSession(); toast("Display name updated!","success"); render(); return; }
  db.ref(ACCOUNTS_REF+"/"+USER.username+"/name").set(newName).then(()=>{
    USER.name = newName;
    persistSession();
    toast("✅ Display name updated!","success");
    document.getElementById("s-username") && (document.getElementById("m-username").textContent = newName);
    render();
  }).catch(()=>setError("edit-name-err","Failed to update. Try again."));
}
